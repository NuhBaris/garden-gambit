...
{
    public sealed class MacaquePetTriggerSourceFactory
        : ICombatPetTriggerSourceFactory
    {
        ...
        public MacaquePetTriggerSourceFactory(
            ...
            CombatAttackGainResolver attackGainResolver)
        {
            if (!petDefinitionId.IsValid)
            {
                throw new ArgumentException(
                    "Macaque factory requires a valid Pet DefinitionId.",
            ...
            CombatPetState pet)
        {
            if (side != CombatSide.Player && side != CombatSide.Enemy)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(side),
                    side,
                    "Macaque source factory requires Player or Enemy side.");
            }
            ...
            {
                throw new ArgumentException(
                    "Pet DefinitionId does not match the Macaque factory registration.",
                    nameof(pet));
            }
