...
{
    public sealed class AlpacaPetTriggerSourceFactory
        : ICombatPetTriggerSourceFactory
    {
        ...
        public AlpacaPetTriggerSourceFactory(
            ...
            CombatAttackGainResolver attackGainResolver)
        {
            if (!petDefinitionId.IsValid)
            {
                throw new ArgumentException(
                    "Alpaca factory requires a valid Pet DefinitionId.",
            ...
            CombatPetState pet)
        {
            if (side != CombatSide.Player && side != CombatSide.Enemy)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(side),
                    side,
                    "Alpaca source factory requires Player or Enemy side.");
            }
            ...
            {
                throw new ArgumentException(
                    "Pet DefinitionId does not match the Alpaca factory registration.",
                    nameof(pet));
            }
