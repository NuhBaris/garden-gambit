...
{
    public sealed class WombatPetTriggerSourceFactory : ICombatPetTriggerSourceFactory
    {
        ...
        public WombatPetTriggerSourceFactory(
            ...
            CombatAttackGainResolver attackGainResolver)
        {
            ...
            {
                throw new ArgumentException("Wombat factory requires a valid Pet DefinitionId.", nameof(petDefinitionId));
            }
        ...
        public IEnumerable<ICombatTriggerSource> CreateSources(CombatSide side, CombatPetState pet)
        {
            if (side != CombatSide.Player && side != CombatSide.Enemy)
            {
                throw new ArgumentOutOfRangeException(nameof(side), side,
                    "Wombat source factory requires Player or Enemy side.");
            }
            ...
            {
                throw new ArgumentException("Pet DefinitionId does not match the Wombat factory registration.", nameof(pet));
            }

            return new ICombatTriggerSource[]
            {
                new WombatPetTriggerSource(side, pet.InstanceId, _usageCommitter, _attackGainResolver)
            };
        }
    }
}
