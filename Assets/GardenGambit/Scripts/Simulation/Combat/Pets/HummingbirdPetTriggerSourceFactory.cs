...
{
    public sealed class
        HummingbirdPetTriggerSourceFactory :
        ICombatPetTriggerSourceFactory
    {
        ...
        private readonly
            CombatAttackGainResolver
            _attackGainResolver;

        public HummingbirdPetTriggerSourceFactory(
                ...
                attackGainResolver)
        {
            if (!petDefinitionId.IsValid)
            {
                throw new ArgumentException(
                    "Hummingbird factory requires a valid " +
                    "Pet DefinitionId.",
                ...
                CombatPetState pet)
        {
                ...
                side != CombatSide.Enemy)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(side),
                    side,
                    "Hummingbird source factory requires " +
                ...
                PetDefinitionId)
            {
                throw new ArgumentException(
                    ...
                    "Hummingbird factory registration.",
                    nameof(pet));
            }

            return new ICombatTriggerSource[]
            {
                new HummingbirdPetTriggerSource(
                    side,
                    pet.InstanceId,
                    _usageCommitter,
                    _attackGainResolver)
