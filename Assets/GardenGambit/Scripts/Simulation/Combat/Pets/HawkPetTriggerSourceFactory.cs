...
{
    public sealed class HawkPetTriggerSourceFactory
        : ICombatPetTriggerSourceFactory
    {
        ...
        public HawkPetTriggerSourceFactory(
            ...
            CombatAttackGainResolver attackGainResolver)
        {
            if (!petDefinitionId.IsValid)
            {
                throw new ArgumentException(
                    "Hawk factory requires a valid Pet DefinitionId.",
            ...
            CombatPetState pet)
        {
            if (side != CombatSide.Player && side != CombatSide.Enemy)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(side),
                    side,
                    "Hawk source factory requires Player or Enemy side.");
            }
            ...
            {
                throw new ArgumentException(
                    "Pet DefinitionId does not match the Hawk factory registration.",
                    nameof(pet));
            }
