...
{
    public sealed class
        GerbilPetTriggerSourceFactoryTests
    {
        [Test]
        ...
        public void Factory_CreatesOneSourceAndResolvesEligibleRankSurvivor(
            CombatSide side,
            BoardRow row)
        {
            var environment =
        ...
        [Test]
        public void Discovery_IgnoresWrongEventAndIneligibleRankCard()
        {
            ...
            var ineligibleSource =
                CreateSource(
                    ...
                    ineligibleEnvironment.Side,
                    ineligibleEnvironment.Pet);

            Assert.That(
                Discover(
                    ineligibleSource,
                    ineligibleEnvironment.State,
                    ineligibleEnvironment
                        .CreateNormalAttackDamage()),
                Is.Empty);

            Assert.That(
                ineligibleEnvironment.UsageRegistry.Count,
                Is.Zero);
        }
        ...
        private static GerbilPetTriggerSourceFactory
            CreateFactory(
                Environment environment)
        {
            return new GerbilPetTriggerSourceFactory(
                environment.Pet.DefinitionId,
                environment.UsageCommitter,
                environment.AttackGainResolver,
                environment.Log);
        ...
        private static GerbilPetTriggerSource
            CreateSource(
                GerbilPetTriggerSourceFactory factory,
                CombatSide side,
                CombatPetState pet)
        {
            var sources =
            ...
            Assert.That(
                sources[0],
                Is.TypeOf<GerbilPetTriggerSource>());

            return (GerbilPetTriggerSource)
                sources[0];
        }
        ...
        private static int CountAttackGainEvents(
            CombatEventLog log)
        {
            var count = 0;
