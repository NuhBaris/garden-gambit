...
{
    public sealed class CombatPetTriggerRuntimeBadgerTests
    {
        [Test]
        public void Catalogue_RegistersStableBadgerIdentityAndSharedDependencies()
        {
            var e = new Environment();
            Assert.That(CombatPetDefinitionIds.BadgerValue, Is.EqualTo("pet.badger"));
            Assert.That(CombatPetDefinitionIds.Badger, Is.EqualTo(new DefinitionId("pet.badger")));
            ...
            var factory = registry.GetFactory(CombatPetDefinitionIds.Badger) as BadgerPetTriggerSourceFactory;
            ...
            Assert.That(registry.Contains(CombatPetDefinitionIds.Koala), Is.True);
            ...
            Assert.That(e.Runtime.FactoryRegistry.Contains(CombatPetDefinitionIds.Badger), Is.False);
            var armorOnly = e.Runtime.FactoryCatalog.CreateRegistry(e.Armor);
            Assert.That(armorOnly.Count, Is.EqualTo(4));
            Assert.That(armorOnly.Contains(CombatPetDefinitionIds.Badger), Is.False);
        }

        [TestCase(CombatSide.Player)]
        [TestCase(CombatSide.Enemy)]
        public void Runtime_BuildsBothBadgerSourcesWithSharedRegistries(CombatSide side)
        {
            ...
            {
                var source = item as BadgerPetTriggerSource;
        ...
        [TestCase(CombatSide.Player, 7, true)]
        [TestCase(CombatSide.Player, 6, false)]
        [TestCase(CombatSide.Enemy, 10, true)]
        [TestCase(CombatSide.Enemy, 11, false)]
        public void FullBattle_ReducesFirstNormalDamageOnlyForEligibleRank(
            CombatSide side, int rank, bool eligible)
        {
            var e = new Environment(side, rank: rank, opponentAttack: 3);
            ...
            Assert.That(e.Target.CurrentHp, Is.EqualTo(eligible ? 8 : 7));
            ...
            Assert.That(e.Runtime.UsageCommitter.HasTriggered(
                e.UpperPet.InstanceId, e.Target.InstanceId), Is.EqualTo(eligible));
            Assert.That(e.Runtime.UsageCommitter.HasTriggered(
                e.LowerPet.InstanceId, e.Target.InstanceId), Is.False);
            Assert.That(e.Runtime.UsageRegistry.Count, Is.EqualTo(eligible ? 1 : 0));
            ...
            Assert.That(hit.Result.IncomingDamage, Is.EqualTo(eligible ? 2 : 3));
            Assert.That(hit.Result.HpDamage, Is.EqualTo(eligible ? 2 : 3));
        }

        [TestCase(CombatSide.Player)]
        [TestCase(CombatSide.Enemy)]
        public void FullBattle_ZeroDamageLeavesBadgerCardUsageAvailable(CombatSide side)
        {
            var e = new Environment(side, rank: 7, opponentAttack: 0);
        ...
        private sealed class Environment
        {
            ...
            public Environment(CombatSide side = CombatSide.Player,
                int rank = 7, int opponentAttack = 3)
            {
                Side = side;
                ...
                Target = Card("test.badger_target", 1, rank, hp: 10, attack: 1);
                var opponent = Card("test.badger_opponent", 101, 2, hp: 1, attack: opponentAttack);
                UpperPet = new CombatPetState(CombatPetDefinitionIds.Badger, new InstanceId(1002));
                LowerPet = new CombatPetState(CombatPetDefinitionIds.Badger, new InstanceId(1001));
                var own = MakeSide(side, Target, _targetPosition, 1);
                var opposing = MakeSide(other, opponent, _opponentPosition, 101);
            ...
            private static CombatCardState Card(string definition, long id, int rank, int hp, int attack) =>
                new CombatCardState(new DefinitionId(definition), new InstanceId(id),
                    new CardRank(rank), CombatCardSeason.Spring, hp, hp, 0, attack);
        }
    }
}
