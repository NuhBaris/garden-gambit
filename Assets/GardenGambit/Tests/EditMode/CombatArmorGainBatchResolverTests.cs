...
{
    public sealed class CombatArmorGainBatchResolverTests
    {
        ...
        public void Batch_PreservesRequestedOrderAndEventAncestry(CombatSide side)
        {
            ...
            var events = e.Resolver.TryApplyArmorGainBatch(
                e.State, e.Root, new[] { e.SecondPosition, e.FirstPosition }, 3);

            Assert.That(events.Count, Is.EqualTo(2));
            Assert.That(e.First.Armor, Is.EqualTo(5));
            Assert.That(e.Second.Armor, Is.EqualTo(7));
            ...
            Assert.That(events[0].PreviousArmor, Is.EqualTo(4));
            Assert.That(events[0].CurrentArmor, Is.EqualTo(7));
            Assert.That(events[1].PreviousArmor, Is.EqualTo(2));
            Assert.That(events[1].CurrentArmor, Is.EqualTo(5));
            ...
            Assert.That(e.First.Attack, Is.EqualTo(1));
            Assert.That(e.Second.Attack, Is.EqualTo(1));
        }

        [Test]
        public void Batch_OverflowOnLastTarget_DoesNotPartiallyApplyAndCanRetry()
        {
            var e = new Environment(secondArmor: int.MaxValue);
            ...
            e.Second.RemoveArmor(1);
            var events = e.Apply();
            Assert.That(e.First.Armor, Is.EqualTo(3));
            Assert.That(e.Second.Armor, Is.EqualTo(int.MaxValue));
        ...
        public void Batch_EmptyOrZero_DoesNotChangeOrAllocate(bool empty)
        {
            ...
            var result = e.Resolver.TryApplyArmorGainBatch(e.State, e.Root,
        ...
        {
            var e = new Environment();
            Assert.Throws<ArgumentException>(() => e.Resolver.TryApplyArmorGainBatch(
        ...
        public void Batch_LastTargetEmpty_RejectsEvenForZeroAmount(int amount)
        {
            ...
            Assert.Throws<InvalidOperationException>(() => e.Resolver.TryApplyArmorGainBatch(
        ...
        public void Batch_InvalidArguments_RejectEvenWithEmptyTargets()
        {
            ...
            Assert.Throws<ArgumentNullException>(() => e.Resolver.TryApplyArmorGainBatch(null, e.Root, empty, 1));
            Assert.Throws<ArgumentNullException>(() => e.Resolver.TryApplyArmorGainBatch(e.State, null, empty, 1));
            Assert.Throws<ArgumentNullException>(() => e.Resolver.TryApplyArmorGainBatch(e.State, e.Root, null, 1));
            Assert.Throws<ArgumentOutOfRangeException>(() => e.Resolver.TryApplyArmorGainBatch(e.State, e.Root, empty, -1));
            var otherReference = new CombatStartedCombatEvent(e.Root.Metadata);
            Assert.Throws<ArgumentException>(() => e.Resolver.TryApplyArmorGainBatch(e.State, otherReference, empty, 1));
            ...
            Assert.Throws<ArgumentException>(() => e.Resolver.TryApplyArmorGainBatch(e.State, unlogged, empty, 1));
            e.AssertUnchanged();
            Assert.That(e.Ids.LastIssuedValue, Is.EqualTo(1));
            Assert.That(e.Sequences.LastIssuedValue, Is.EqualTo(1));
        }
        ...
        public void Batch_MetadataExhaustionOnLastTarget_DoesNotChangeCardsOrLog(bool exhaustIds)
        {
            ...
            var resolver = new CombatArmorGainResolver(factory, e.Log);
            Assert.Throws<InvalidOperationException>(() => resolver.TryApplyArmorGainBatch(
        ...
        public void Batch_LogRejectsLastCandidate_DoesNotAppendFirstOrChangeAnyCard()
        {
            ...
            Assert.Throws<ArgumentException>(() => resolver.TryApplyArmorGainBatch(
                e.State, e.Root, e.Positions, 1));
            Assert.That(e.First.Armor, Is.EqualTo(2));
            Assert.That(e.Second.Armor, Is.EqualTo(4));
        ...
        {
            var e = new Environment();
            var parent = e.Resolver.TryApplyArmorGain(e.State, e.Root, e.FirstPosition, 1);
            e.Second.SetCurrentHpToZero();
            var events = e.Resolver.TryApplyArmorGainBatch(e.State, parent, e.Positions, 1);
            Assert.That(e.First.Armor, Is.EqualTo(4));
            Assert.That(e.Second.Armor, Is.EqualTo(5));
        ...
        private sealed class Environment
        {
            ...
            public Environment(CombatSide side = CombatSide.Player, int secondArmor = 4)
            {
                ...
                Log.Append(Root);
                Resolver = new CombatArmorGainResolver(factory, Log);
            }

            public IReadOnlyList<ArmorGainCombatEvent> Apply()
            {
                return Resolver.TryApplyArmorGainBatch(State, Root, Positions, 1);
            }

            public void AssertUnchanged(int firstArmor = 2, int secondArmor = 4)
            {
                Assert.That(First.Armor, Is.EqualTo(firstArmor));
                Assert.That(Second.Armor, Is.EqualTo(secondArmor));
                Assert.That(Log.Count, Is.EqualTo(1));
                Assert.That(Log.Events[0], Is.SameAs(Root));
            }

            private static CombatCardState Card(long id, int armor)
            {
                return new CombatCardState(new DefinitionId("test.batch_target"), new InstanceId(id),
                    new CardRank(2), CombatCardSeason.Autumn, 5, 3, armor, 1);
            }
