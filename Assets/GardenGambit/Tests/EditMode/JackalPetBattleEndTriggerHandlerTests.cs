...
{
    public sealed class JackalPetBattleEndTriggerHandlerTests
    {
        [TestCase(true)]
        [TestCase(false)]
        public void Constructor_RejectsMissingDependency(bool missingUsage)
        {
            ...
            Assert.Throws<ArgumentNullException>(() => new JackalPetBattleEndTriggerHandler(
        ...
        {
            var e = new Environment();
            Assert.Throws<ArgumentOutOfRangeException>(() => new JackalPetBattleEndTriggerHandler(
        ...
        {
            var e = new Environment();
            Assert.Throws<ArgumentException>(() => new JackalPetBattleEndTriggerHandler(
        ...
        [TestCase(4)]
        [TestCase(5)]
        public void LivingCount_AwardsEachLivingOddRankCardWithoutMinimumCount(int livingCount)
        {
        ...
        [Test]
        public void TwoJackals_KeepIndependentUsesForTheirRows()
        {
        ...
        public void FinalRankBonus_AddsToExistingModifierAfterStructuralContribution()
        {
            ...
            card.SetRank(new CardRank(13));
            e.Modifiers.AddModifier(card.InstanceId, 3);
            e.Handler(BoardRow.Front).Resolve(e.State, e.Event);
            Assert.That(card.Rank.Value, Is.EqualTo(13));
            Assert.That(e.Modifiers.GetTotalModifier(card.InstanceId), Is.EqualTo(5));
            Assert.That(new CombatFinalRankContributionResolver(e.Modifiers).Resolve(card.InstanceId, 39), Is.EqualTo(44));
        ...
        [TestCase(13, true)]
        [TestCase(14, false)]
        public void RankRange_OnlyOddCurrentRanksReceiveBonus(int rank, bool eligible)
        {
        ...
        public void Resolve_UsesRankAtResolutionTime(int before, int after)
        {
            ...
            var handler = e.Handler(BoardRow.Front);
            Assert.That(handler.CanTrigger(e.State, e.Event), Is.EqualTo(before == 3));
            card.SetRank(new CardRank(after));
            handler.Resolve(e.State, e.Event);
            Assert.That(e.Modifiers.GetTotalModifier(card.InstanceId), Is.EqualTo(after == 3 ? 2 : 0));
            Assert.That(e.Usage.UsageRegistry.Count, Is.EqualTo(after == 3 ? 1 : 0));
        }
        ...
        public void ExistingFinalRankModifier_DoesNotChangeParityCondition(int rank, int expectedModifier)
        {
            ...
            Assert.That(e.Usage.HasTriggered(e.UpperPet.InstanceId, card.InstanceId), Is.EqualTo(rank == 3));
        }
        ...
        private sealed class Environment
        {
            ...
            public JackalPetBattleEndTriggerHandler Handler(BoardRow row) => new JackalPetBattleEndTriggerHandler(
            ...
            private static CombatPetState Pet(long id) => new CombatPetState(new DefinitionId("test.pet.jackal"), new InstanceId(id));
            ...
            private static CombatSideState CreateSide(CombatSide side, int frontLiving, int backLiving)
            {
                ...
                for (var column = 5; column >= 1; column--)
                {
                    foreach (var row in new[] { BoardRow.Back, BoardRow.Front })
                    {
                        ...
                        var card = new CombatCardState(new DefinitionId("test.jackal.card"), new InstanceId(id),
                            new CardRank(3), 5, column <= (row == BoardRow.Front ? frontLiving : backLiving) ? 5 : 0, 0, 2);
                        cards.Add(card);
