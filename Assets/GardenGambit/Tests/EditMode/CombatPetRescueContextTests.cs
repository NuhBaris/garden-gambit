...
namespace GardenGambit.Tests.EditMode
{
        ...
        CombatPetRescueContextTests
    {
        ...
        [Test]
        public void
            Constructor_WithOpposingSideRescue_AllowsPetToObserveEnemyRescue()
        {
            var state =
                CreateEmptyState();

            var enemyRescueEvent =
                CreateRescueEvent(
                    CombatSide.Enemy);
        ...
        [TestCase(false)]
        [TestCase(true)]
        public void GetAffectedRow_WithEnemyPet_UsesEnemyPlacement(bool lower)
        {
            var upperPet = CreatePet("test.rescue_pet", 2002);
            var lowerPet = CreatePet("test.rescue_pet", 2001);
            var state = CreateState(Array.Empty<CombatPetState>(), new[] { upperPet, lowerPet });
            var context = new CombatPetRescueContext(state, CombatSide.Enemy, CreateRescueEvent(CombatSide.Player));
            Assert.That(context.GetAffectedRow(lower ? lowerPet : upperPet),
                Is.EqualTo(lower ? BoardRow.Back : BoardRow.Front));
            Assert.That(context.SidePetState, Is.SameAs(state.EnemyPets));
        }

        [Test]
        public void GetAffectedRow_WithUnregisteredPet_Throws()
        {
            var context = new CombatPetRescueContext(CreateEmptyState(), CombatSide.Player,
                CreateRescueEvent(CombatSide.Player));
            Assert.Throws<ArgumentException>(() => context.GetAffectedRow(CreatePet("test.unknown", 9001)));
        }
