using System;
using GardenGambit.Domain.Combat;
using GardenGambit.Simulation.Combat;
using NUnit.Framework;

namespace GardenGambit.Tests.EditMode
{
    public sealed class CombatEventIdAllocatorTests
    {
        [Test]
        public void NewAllocator_StartsBeforeFirstId()
        {
            var allocator =
                new CombatEventIdAllocator();

            Assert.That(
                allocator.LastIssuedValue,
                Is.Zero);

            Assert.That(
                allocator.CanAllocate,
                Is.True);
        }

        [Test]
        public void Allocate_FirstCall_ReturnsOne()
        {
            var allocator =
                new CombatEventIdAllocator();

            var eventId =
                allocator.Allocate();

            Assert.That(
                eventId,
                Is.EqualTo(new CombatEventId(1)));

            Assert.That(
                allocator.LastIssuedValue,
                Is.EqualTo(1));
        }

        [Test]
        public void Allocate_MultipleCalls_ReturnsSequentialIds()
        {
            var allocator =
                new CombatEventIdAllocator();

            var first = allocator.Allocate();
            var second = allocator.Allocate();
            var third = allocator.Allocate();

            Assert.That(
                first,
                Is.EqualTo(new CombatEventId(1)));

            Assert.That(
                second,
                Is.EqualTo(new CombatEventId(2)));

            Assert.That(
                third,
                Is.EqualTo(new CombatEventId(3)));
        }

        [Test]
        public void Constructor_WithLastIssuedValue_ResumesAfterValue()
        {
            var allocator =
                new CombatEventIdAllocator(50);

            var eventId =
                allocator.Allocate();

            Assert.That(
                eventId,
                Is.EqualTo(new CombatEventId(51)));
        }

        [Test]
        public void Constructor_WithNegativeLastIssuedValue_Throws()
        {
            Assert.Throws<ArgumentOutOfRangeException>(
                () => _ =
                    new CombatEventIdAllocator(-1));
        }

        [Test]
        public void Allocate_AtMaximumValue_ExhaustsWithoutOverflow()
        {
            var allocator =
                new CombatEventIdAllocator(
                    long.MaxValue - 1);

            var finalEventId =
                allocator.Allocate();

            Assert.That(
                finalEventId,
                Is.EqualTo(
                    new CombatEventId(long.MaxValue)));

            Assert.That(
                allocator.CanAllocate,
                Is.False);

            Assert.Throws<InvalidOperationException>(
                () => allocator.Allocate());

            Assert.That(
                allocator.LastIssuedValue,
                Is.EqualTo(long.MaxValue));
        }
    }
}